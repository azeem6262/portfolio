...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  const dispRef = useRef<SVGElement | null>(null);
  const filterElRef = useRef<SVGFilterElement | null>(null);
  ...
  const updateAnim = React.useCallback(() => {
    ...
    if (dyAnimsRef.current.length === 0) {
    ...
    if (dxAnimsRef.current.length === 0) {
    ...
    if (!dispRef.current) {
      dispRef.current = svg.querySelector("feDisplacementMap");
    }
    if (!filterElRef.current) {
      filterElRef.current = svg.querySelector<SVGFilterElement>(
        `#${CSS.escape(filterId)}`,
      );
    }

    const dyAnims = dyAnimsRef.current;
    if (dyAnims.length >= 2) {
      dyAnims[0].setAttribute("values", `${height}; 0`);
      dyAnims[1].setAttribute("values", `0; -${height}`);
    }

    const dxAnims = dxAnimsRef.current;
    if (dxAnims.length >= 2) {
    ...
    const filterEl = filterElRef.current;
    if (filterEl) {
